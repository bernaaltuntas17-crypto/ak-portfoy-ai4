...
def load_data():
    ...
    # Backup Verisi (Sunum Hayat Kurtaran)
    backup = {
...
if analyze_btn:
    with st.spinner("Veriler Hesaplanıyor..."):
        ...
        # Grafik değerlerini bulunan fon adedine göre ayarlıyoruz (HATANIN ÇÖZÜMÜ BURASI)
        count = len(f_kodlar)
        ...
        if API_KEY:
            prompt = f"Ak Portföy Analisti olarak {amount_val} yatırım için {ans_risk} risk profilinde derin analiz yaz: {df.to_string()}"
            try:
                url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={API_KEY}"
                res = requests.post(url, json={"contents": [{"parts": [{"text": prompt}]}]}, timeout=15)
                if res.status_code == 200: st.markdown(res.json()['candidates'][0]['content']['parts'][0]['text'])
